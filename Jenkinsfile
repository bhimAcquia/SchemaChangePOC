    ...
    stages {
        stage('Run schemachange') {
            steps {
                ...
                sh "schemachange -f snowflake/tenants/tenant_803 -a ${SF_ACCOUNT} -u ${SF_USERNAME} -r ${SF_ROLE} -w ${SF_WAREHOUSE} -d ${SF_DATABASE} -c ${SF_DATABASE}.SCHEMACHANGE.CHANGE_HISTORY_803 --create-change-history-table"
            }
        }
    }
}